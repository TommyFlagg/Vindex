...
function vindexFinnMengde(tekst) {
  ...
  const treff = t.match(
    /(\d+(?:\.\d+)?)\s*(l[oø]pemeter|meter|lm\b|m2\b|m²|kvadratmeter|stk\b|stykk|vindu(?:er|a)?\b)/
  );
...
function vindexEtikettfelt(tekst) {
  ...
  for (const rå of linjer) {
    const linje = rå.replace(/ /g, " ").trimEnd();
    const deling = linje.match(/^\s*\*{0,2}([^:：]{1,40})[:：]\s*(.*)$/);
    const felt = deling ? feltFor(deling[1].replace(/\*/g, "")) : null;

    if (felt) {
      aktiv = felt;
      const verdi = deling[2].trim();
      funne[felt] = verdi;
      // Står verdien tom, held vi feltet ope for linja under.
      if (verdi) aktiv = felt;
      continue;
    }

    // Framhald av førre felt: berre kommentarfeltet får fleire linjer, og
    // berre så lenge det ikkje kjem ei ny etikettlinje.
    if (aktiv && linje.trim()) {
      if (aktiv === "kommentar" || !funne[aktiv]) {
        funne[aktiv] = (funne[aktiv] ? funne[aktiv] + "\n" : "") + linje.trim();
      } else {
        aktiv = null;
      }
    } else if (!linje.trim()) {
      // Tom linje avsluttar alt anna enn kommentaren.
...
function vindexLesLead(råtekst) {
  ...
  const ut = {
    navn: (merka.navn || "").replace(/<[^>]*>/g, "").trim(),
  ...
  // Postnummer: etiketten først, så teksten.
  const merktPostnr = (merka.postnr || "").match(/\b\d{4}\b/);
