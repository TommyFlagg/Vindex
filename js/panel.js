...
function manadsdiagram(iAar, iFjor, aar, fjorAar) {
  ...
  const stolpar = iAar
    .map((m, i) => {
      ...
      return `<g class="stolpegruppe" data-manad="${vindexT(m.navn)}" data-sum="${m.sum}" data-fjor="${fjor}">
  ...
  el.innerHTML = `
    ...
    <div class="panel-rutenett">
      ...
      <div class="card tilt" data-tilt>
        ...
        <p class="ko-status ko-${ko.niva}">${vindexT(ko.tekst)}</p>
        ${kapasitetsmalar(ko)}
      ...
      <div class="table-scroll mt-1" style="border:none">
        <table class="data" style="min-width:420px">
          ...
          <tbody>${perSeljar
            .map(
              (r) => `<tr>
                <td><strong>${vindexT(r.seljar.navn)}</strong>${r.seljar.type === "forhandler" ? ' <span class="merke merke-liten">forhandler</span>' : ""}</td>
                <td>${vindexT(r.seljar.sted)}</td>
                <td>${r.tal}</td>
            ...
            ${rangering
              .map(
                (r, i) => `<tr class="${r.seljar.id === brukar.uid ? "meg" : ""}">
                  ...
                  <td><strong>${vindexT(r.seljar.navn)}</strong>${r.seljar.id === brukar.uid ? ' <span class="merke merke-liten">deg</span>' : ""}
                    ${r.seljar.type === "forhandler" ? '<br><span class="hint">forhandler</span>' : ""}
                    ${r.seljar.sted ? `<br><span class="hint">${vindexT(r.seljar.sted)}</span>` : ""}</td>
                  <td>
  ...
  const banar = VINDEX_KART.fylke
    .map((f) => {
      ...
      return `<path d="${f.bane}" fill="${rampe()[steg]}"
        ...
        ${interaktiv ? `tabindex="0" role="button" aria-label="${vindexT(f.navn)}: ${verdi} ${eining}"` : 'aria-hidden="true"'}>
        ${interaktiv ? `<title>${vindexT(f.navn)} — ${verdi} ${eining}</title>` : ""}
      </path>`;
    })
    .join("");
  ...
  el.innerHTML = `
    <div class="panel-topp">
      ...
      <div class="fanerad" id="kartFaner">
        ${KART_VISNINGAR.map((v) => `<button class="fane ${v.id === "kunder" ? "aktiv" : ""}" data-kartfane="${v.id}">${vindexT(v.navn)}</button>`).join("")}
      </div>
    </div>
  ...
  function teiknSide(per) {
      ...
      const liste = (tittel, rader, klasse) =>
        ...
        (rader.length
          ? `<ul class="grunnliste">${rader
              .map(
                (r) => `<li><span>${vindexT(r.navn)}</span><span class="grunntal ${klasse}">${r.tal}</span></li>`
              )
              .join("")}</ul>`
    ...
    if (valtFylke) {
      ...
      side.innerHTML = `
        ...
        <h4>Dekkes av</h4>
        ${dekker.length ? `<ul class="enkelliste">${dekker.map((s) => `<li>${vindexT(s.navn)}</li>`).join("")}</ul>`
                        : '<p class="hint" style="color:var(--bad)">Ingen selger dekker dette fylket.</p>'}
        <h4>Siste saker</h4>
        ${sisteLeads.length
          ? `<ul class="enkelliste">${sisteLeads
              .map((l) => `<li><button class="lenkeknapp" data-lead="${l.id}">${vindexT((l.kunde || {}).navn)}</button>
